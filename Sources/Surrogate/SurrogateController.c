...
const CFStringRef kMOMRestrictToSpecifiedHost =
    CFSTR("restrict_to_specified_host");
...
const CFStringRef kMOMRecoveryFirmwareVersion =
    CFSTR("recovery_firmware_version");
...
const CFStringRef kMOMRestrictToSpecifiedHost =
    CFSTR("kMOMRestrictToSpecifiedHost");
...
const CFStringRef kMOMRecoveryFirmwareVersion =
    CFSTR("kMOMRecoveryFirmwareVersion");
const CFStringRef kMOMLocalInterfaceAddress =
    CFSTR("kMOMLocalInterfaceAddress");
#endif /* !MOM_STREAMDECK_PLUGIN */

static void *__MOMControllerRetainStrong(MOMControllerRef controller) {
...
static void __MOMControllerDealloc(MOMControllerRef controller) {
...
static void __MOMControllerReleaseStrong(MOMControllerRef controller) {
...
static const void *_MOMControllerRetain_CFAllocatorContext(const void *value) {
    return __MOMControllerRetainStrong((MOMControllerRef)value);
}

static void _MOMControllerRelease_CFAllocatorContext(const void *value) {
...
_MOMControllerCopyDescription_CFAllocatorContext(const void *value) {
    ...
    return CFStringCreateWithFormat(CFGetAllocator(controller), NULL,
                                    CFSTR("<MOMController %p"), controller);
}
...
static void handleTcpConnect(CFSocketRef socket,
    ...
    *nativeSocketHandle = *((CFSocketNativeHandle *)data);
    ...
    _MOMResolveHostRestrictionAndPerform(controller, ^void(
                                             MOMControllerRef controller,
                                             CFArrayRef restrictAddressList) {
      ...
      if (peerAddress == NULL ||
          ((struct sockaddr *)name)->sa_family != AF_INET) {
      ...
      if (inet_ntop(PF_INET, &sin->sin_addr, buffer, sizeof(buffer)) == NULL) {
          ...
          peerName = CFStringCreateWithFormat(kCFAllocatorDefault, NULL,
                                              CFSTR("%s"), buffer);
      }

      if (restrictAddressList &&
      ...
      if (setsockopt(*nativeSocketHandle, IPPROTO_TCP, TCP_NODELAY,
                     (void *)&yes, sizeof(yes)) != 0) {
          int savedError = errno;

          _MOMDebugLog(
              CFSTR("failed to set TCP_NODELAY on socket for peer %@: %s"),
              peerName, strerror(savedError));
      }

      CFStreamCreatePairWithSocket(kCFAllocatorDefault, *nativeSocketHandle,
                                   &readStream, &writeStream);
      if (!_MOMHandleConnectionFromNewAddress(controller, peerAddress, peerName,
                                              readStream, writeStream)) {
          close(*nativeSocketHandle);
          goto out;
      }
...
static CFSocketRef createTcpListenerSocket(MOMControllerRef controller) {
    ...
    tcpSocket = CFSocketCreate(kCFAllocatorDefault, PF_INET, SOCK_STREAM,
                               IPPROTO_TCP, kCFSocketAcceptCallBack,
                               handleTcpConnect, &socketContext);
    if (tcpSocket == NULL) {
        return NULL;
    }

    if (setsockopt(CFSocketGetNative(tcpSocket), SOL_SOCKET, SO_REUSEADDR,
                   (void *)&yes, sizeof(yes)) != 0) {
        int savedError = errno;

        _MOMDebugLog(CFSTR("failed to set SO_REUSEADDR on socket: %s"),
                     strerror(savedError));
    }
    ...
    sin.sin_addr.s_addr =
        sinLocalAddress ? sinLocalAddress->sin_addr.s_addr : INADDR_ANY;
...
static void setDefaultOptionInt32(MOMControllerRef controller,
                                  CFTypeRef option,
                                  int32_t value) {
...
static void setDefaultOptionString(MOMControllerRef controller,
                                   CFStringRef option,
...
_Nullable MOMControllerRef MOMControllerCreate(
    ...
    if (options == NULL) {
        controller->options = CFDictionaryCreateMutable(
            kCFAllocatorDefault, 0, &kCFTypeDictionaryKeyCallBacks, NULL);
    } else {
        controller->options =
    ...
    setDefaultOptionString(controller, kMOMSystemTypeAndVersion,
                           CFSTR("710100A   171127"));
    setDefaultOptionString(controller, kMOMCPUFirmwareTag, CFSTR("cpufw"));
    setDefaultOptionString(controller, kMOMCPUFirmwareVersion,
                           CFSTR("1.0.0.2"));
    setDefaultOptionString(controller, kMOMRecoveryFirmwareTag,
                           CFSTR("recovery"));
    setDefaultOptionString(controller, kMOMRecoveryFirmwareVersion,
                           CFSTR("1.0.0.2"));
...
void MOMControllerRelease(MOMControllerRef controller) {
    __MOMControllerReleaseStrong(controller);
}

MOMControllerRef MOMControllerRetain(MOMControllerRef controller) {
    return __MOMControllerRetainStrong(controller);
}

static MOMStatus enqueueEvent(MOMControllerRef controller,
                              MOMEvent event,
...
MOMStatus _MOMControllerEnqueueNotification(MOMControllerRef controller,
                                            MOMEvent event,
                                            CFArrayRef eventParams) {
    return enqueueEvent(controller, event | kMOMEventTypeDeviceNotification,
                        eventParams);
}

MOMStatus MOMControllerNotify(MOMControllerRef controller,
                              MOMEvent event,
                              CFArrayRef eventParams) {
    MOMStatus status;
...
MOMStatus MOMControllerNotifyDeferred(MOMControllerRef controller,
                                      MOMEvent event,
                                      CFArrayRef eventParams) {
    return _MOMControllerEnqueueNotification(controller, event, eventParams);
}

MOMStatus MOMControllerSendDeferred(MOMControllerRef controller) {
    return _MOMControllerSendToPeers(controller);
}

CFMutableDictionaryRef MOMControllerGetOptions(MOMControllerRef controller) {
    return controller->options;
}

MOMStatus MOMControllerBeginDiscoverability(MOMControllerRef controller) {
    ...
    socketSource =
        CFSocketCreateRunLoopSource(kCFAllocatorDefault, tcpSocket, 0);
    ...
    socketSource =
        CFSocketCreateRunLoopSource(kCFAllocatorDefault, udpSocket, 0);
    CFRunLoopAddSource(controller->runLoop, socketSource,
                       kCFRunLoopCommonModes);
    CFRelease(socketSource);
...
MOMStatus MOMControllerEndDiscoverability(MOMControllerRef controller) {
