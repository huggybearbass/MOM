...
static inline uint16_t getPeerPort(const MOMPeerContext *peerContext) {
    if (peerContext->peerAddress) {
        struct sockaddr_in *sin =
            (struct sockaddr_in *)CFDataGetBytePtr(peerContext->peerAddress);
...
static void *__MOMPeerContextRetainStrong(MOMPeerContext *peerContext) {
...
static void __MOMPeerContextDealloc(MOMPeerContext *peerContext) {
...
static void __MOMPeerContextReleaseStrong(MOMPeerContext *peerContext) {
...
void _MOMPeerContextRetain(MOMPeerContext *peerContext) {
    __MOMPeerContextRetainStrong(peerContext);
}

void _MOMPeerContextRelease(MOMPeerContext *peerContext) {
    __MOMPeerContextReleaseStrong(peerContext);
}

static const void *
_MOMPeerContextRetain_CFArrayCallBacks(CFAllocatorRef allocator,
                                       const void *value) {
    return __MOMPeerContextRetainStrong((MOMPeerContext *)value);
}

static void _MOMPeerContextRelease_CFArrayCallBacks(CFAllocatorRef allocator,
                                                    const void *value) {
...
_MOMPeerContextCopyDescription_CFArrayCallBacks(const void *value) {
    ...
    return CFStringCreateWithFormat(
        ...
        CFSTR("<MOMPeerContext %p {address = %@:%u}>"), peerContext,
        peerContext->peerName, getPeerPort(peerContext));
}

static Boolean _MOMPeerContextEqual_CFArrayCallBacks(const void *v1,
                                                     const void *v2) {
    return v1 == v2;
}

void _MOMSetPeerPortStatus(MOMControllerRef controller,
...
MOMPortStatus _MOMGetPeerPortStatus(MOMControllerRef controller,
                                    MOMPeerContext *peerContext) {
    return peerContext->portStatus;
}

static bool _MOMPeerWriteBuffer(MOMPeerContext *peerContext) {
    bool ret;

    assert(peerContext->writeBuffer);

    while (CFWriteStreamCanAcceptBytes(peerContext->writeStream) &&
           peerContext->bytesWritten <
               CFDataGetLength(peerContext->writeBuffer)) {
        peerContext->bytesWritten +=
            CFWriteStreamWrite(peerContext->writeStream,
                               CFDataGetBytePtr(peerContext->writeBuffer) +
                                   peerContext->bytesWritten,
                               CFDataGetLength(peerContext->writeBuffer) -
                                   peerContext->bytesWritten);
    }
...
static void handleMessage(MOMPeerContext *peerContext, CFStringRef messageBuf) {
    ...
    if (_MOMParseMessageString(messageBuf, &event, &eventParams, &errorReply) ==
            kMOMStatusSuccess &&
        (event & kMOMEventTypeHostAny)) {
        _MOMProcessEvent(peerContext->controller, peerContext, event,
                         eventParams);
    } else if (errorReply) {
        _MOMControllerEnqueueMessage(peerContext->controller, peerContext,
                                     errorReply);
    }
...
static void handleMessages(MOMPeerContext *peerContext, CFArrayRef messages) {
    if (messages != NULL) {
        CFIndex i;
...
peerContextClose(MOMPeerContext *peerContext, uint32_t flags, CFErrorRef err) {
    ...
    if ((flags & kMOMPeerContextCloseReadStream) &&
        CFReadStreamGetStatus(peerContext->readStream) !=
            kCFStreamStatusClosed) {
        if (_MOMPeerIsMaster(controller, peerContext))
            _MOMSetMasterPeer(controller, NULL);
        if (_MOMGetPeerPortStatus(controller, peerContext) >=
            kMOMPortStatusOpen)
            _MOMSetPeerPortStatus(controller, peerContext, kMOMPortStatusClosed,
                                  err);

        CFReadStreamUnscheduleFromRunLoop(peerContext->readStream,
                                          peerContext->controller->runLoop,
    ...
    if ((flags & kMOMPeerContextCloseWriteStream) &&
        CFWriteStreamGetStatus(peerContext->writeStream) !=
            kCFStreamStatusClosed) {
        CFWriteStreamUnscheduleFromRunLoop(peerContext->writeStream,
                                           peerContext->controller->runLoop,
                                           kCFRunLoopCommonModes);
...
static CFArrayRef parseMessages(MOMPeerContext *peerContext) {
    // XXX support \r\n
    if (CFStringGetCharacterAtIndex(peerContext->readBuffer,
                                    CFStringGetLength(peerContext->readBuffer) -
                                        1) == '\r')
        return CFStringCreateArrayBySeparatingStrings(
            kCFAllocatorDefault, peerContext->readBuffer, CFSTR("\r"));

    return NULL;
}

static void readStreamCallback(CFReadStreamRef readStream,
    ...
    case kCFStreamEventHasBytesAvailable: {
        ...
        if (peerContext->readBuffer == NULL)
            peerContext->readBuffer =
                CFStringCreateMutable(kCFAllocatorDefault, bytesRead);

        if (bytesRead > 0) {
            CFStringRef readBuffer =
                CFStringCreateWithBytes(kCFAllocatorDefault, buf, bytesRead,
...
static void writeStreamCallback(CFWriteStreamRef writeStream,
                                CFStreamEventType eventType,
                                void *userInfo) {
    MOMPeerContext *peerContext = (MOMPeerContext *)userInfo;
    CFErrorRef err = NULL;
...
bool _MOMHandleConnectionFromNewAddress(MOMControllerRef controller,
    ...
    CFReadStreamSetProperty(peerContext->readStream,
                            kCFStreamPropertyShouldCloseNativeSocket,
                            kCFBooleanTrue);
    // this would once have allowed us to run in the background
    // CFReadStreamSetProperty(peerContext->readStream,
    // kCFStreamNetworkServiceTypeVoIP, kCFBooleanTrue);
    CFReadStreamSetClient(
        peerContext->readStream,
        kCFStreamEventOpenCompleted | kCFStreamEventHasBytesAvailable |
            kCFStreamEventErrorOccurred | kCFStreamEventEndEncountered,
        readStreamCallback, &streamContext);
    CFReadStreamScheduleWithRunLoop(peerContext->readStream,
                                    controller->runLoop, kCFRunLoopCommonModes);
    CFReadStreamOpen(peerContext->readStream);

    CFRetain(writeStream);
    peerContext->writeStream = writeStream;

    CFWriteStreamSetProperty(peerContext->writeStream,
                             kCFStreamPropertyShouldCloseNativeSocket,
                             kCFBooleanTrue);
    CFWriteStreamSetClient(
        peerContext->writeStream,
        kCFStreamEventOpenCompleted | kCFStreamEventCanAcceptBytes |
            kCFStreamEventErrorOccurred | kCFStreamEventEndEncountered,
        writeStreamCallback, &streamContext);
    CFWriteStreamScheduleWithRunLoop(
        peerContext->writeStream, controller->runLoop, kCFRunLoopCommonModes);
    ...
    _MOMDebugLog(CFSTR("new connection %p {address = %@:%u}"), peerContext,
                 peerContext->peerName, getPeerPort(peerContext));
...
static void possiblyExpirePeerContext(const void *value, void *context) {
    ...
    if (peerContext->lastActivity + controller->aliveTime < now) {
        _MOMDebugLog(
            CFSTR("expiring connection %p {address = %@:%u, idle = %d}"),
            peerContext, peerContext->peerName, getPeerPort(peerContext),
            peerContext->lastActivity ? now - peerContext->lastActivity : -1);
        peerContextClose(peerContext,
                         kMOMPeerContextCloseReadStream |
                             kMOMPeerContextCloseWriteStream,
                         NULL);
    } else {
...
bool _MOMSetAliveTime(MOMControllerRef controller, int32_t aliveTime) {
    ...
    timer = CFRunLoopTimerCreateWithHandler(
        kCFAllocatorDefault, 0, aliveTime, 0, 0, ^(CFRunLoopTimerRef timer) {
          CFMutableArrayRef nonExpiredPeers =
              _MOMCreatePeerContextArray(CFArrayGetCount(controller->peers));

          CFArrayApplyFunction(
              controller->peers,
              CFRangeMake(0, CFArrayGetCount(controller->peers)),
              possiblyExpirePeerContext, nonExpiredPeers);
    ...
    if (controller->peerExpiryTimer) {
        CFRunLoopRemoveTimer(controller->runLoop, controller->peerExpiryTimer,
                             kCFRunLoopCommonModes);
    ...
    CFRunLoopAddTimer(controller->runLoop, controller->peerExpiryTimer,
                      kCFRunLoopCommonModes);

    return true;
}

CFMutableArrayRef _MOMCreatePeerContextArray(CFIndex capacity) {
    ...
    return CFArrayCreateMutable(kCFAllocatorDefault, capacity,
                                &peerContextCallBacks);
}

void _MOMControllerEnqueueMessage(MOMControllerRef controller,
                                  MOMPeerContext *peerContext,
                                  CFDataRef messageBuf) {
    assert(peerContext->writeBuffer);
    CFDataAppendBytes(peerContext->writeBuffer, CFDataGetBytePtr(messageBuf),
                      CFDataGetLength(messageBuf));
}

MOMStatus _MOMControllerSendToPeers(MOMControllerRef controller) {
    CFIndex i;
    ...
    for (i = 0; i < CFArrayGetCount(controller->peers); i++) {
        MOMPeerContext *peerContext =
            (MOMPeerContext *)CFArrayGetValueAtIndex(controller->peers, i);
...
bool _MOMPeerIsMaster(MOMControllerRef controller,
                      MOMPeerContext *peerContext) {
    return peerContext == controller->peerMaster;
}

void _MOMSetMasterPeer(MOMControllerRef controller,
                       MOMPeerContext *peerContext) {
...
void _MOMInvalidatePeers(MOMControllerRef controller) {
    CFIndex i;
    ...
    for (i = 0; i < CFArrayGetCount(controller->peers); i++) {
        ...
        _MOMDebugLog(CFSTR("invalidating connection %p {address = %@:%u}"),
                     peerContext, peerContext->peerName,
