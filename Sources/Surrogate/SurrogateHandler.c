...
/*
 * A note on writing handlers: the parameter list is treated as a stack. By
 * default all parameters from the request are echoed back to the host (if the
 * host expects a reply).
 *
 * The returned status is inserted at top of the stack (index 0): this need not
 * be done by the handler.
 *
 * Return values should be pushed in reverse order onto the stack.
 *
 * If a request parameter is not needed, it should be popped, otherwise it can
 * be left; usually they are echoed back to the host.
 */

// HostRequest
static MOMStatus aliveRequest(MOMControllerRef controller,
...
// HostRequest
static MOMStatus getDeviceID(MOMControllerRef controller,
                             MOMPeerContext *peerContext,
                             MOMEvent event,
                             CFMutableArrayRef params) {
...
// HostNotification
static MOMStatus setDeviceID(MOMControllerRef controller,
                             MOMPeerContext *peerContext,
                             MOMEvent event,
                             CFMutableArrayRef params) {
    int32_t deviceID = 0;
    ...
    deviceIDNumber =
        CFNumberCreate(kCFAllocatorDefault, kCFNumberSInt32Type, &deviceID);
...
// HostRequest
static MOMStatus getHardwareConfig(MOMControllerRef controller,
                                   MOMPeerContext *peerContext,
                                   MOMEvent event,
                                   CFMutableArrayRef params) {
    int32_t version = 0;
...
// HostRequest
static MOMStatus getSoftwareVersion(MOMControllerRef controller,
                                    MOMPeerContext *peerContext,
                                    MOMEvent event,
                                    CFMutableArrayRef params) {
    int32_t version;
...
// HostRequest
static MOMStatus getDeviceInfo(MOMControllerRef controller,
                               MOMPeerContext *peerContext,
                               MOMEvent event,
                               CFMutableArrayRef params) {
...
// HostRequest
static MOMStatus getMaster(MOMControllerRef controller,
                           MOMPeerContext *peerContext,
                           MOMEvent event,
                           CFMutableArrayRef params) {
...
// HostNotification
static MOMStatus setMaster(MOMControllerRef controller,
                           MOMPeerContext *peerContext,
                           MOMEvent event,
                           CFMutableArrayRef params) {
    int32_t master;
    ...
    _MOMSetPeerPortStatus(
        controller, peerContext,
        master ? kMOMPortStatusConnected : kMOMPortStatusReady, NULL);

    return kMOMStatusSuccess;
}

// HostRequest
static MOMStatus getAliveTime(MOMControllerRef controller,
                              MOMPeerContext *peerContext,
                              MOMEvent event,
                              CFMutableArrayRef params) {
...
// HostNotification
static MOMStatus setAliveTime(MOMControllerRef controller,
                              MOMPeerContext *peerContext,
                              MOMEvent event,
                              CFMutableArrayRef params) {
    int32_t aliveTime = 0;
    ...
    if (_MOMGetPeerPortStatus(controller, peerContext) < kMOMPortStatusReady)
        _MOMSetPeerPortStatus(controller, peerContext, kMOMPortStatusReady,
                              NULL);
...
// HostRequest
static MOMStatus getIPAddress(MOMControllerRef controller,
                              MOMPeerContext *peerContext,
                              MOMEvent event,
                              CFMutableArrayRef params) {
...
// HostRequest
static MOMStatus setIPAddress(MOMControllerRef controller,
                              MOMPeerContext *peerContext,
                              MOMEvent event,
                              CFMutableArrayRef params) {
...
// HostRequest
static MOMStatus getKeyMode(MOMControllerRef controller,
                            MOMPeerContext *peerContext,
                            MOMEvent event,
                            CFMutableArrayRef params) {
    int32_t keyNumber;
...
// HostRequest
static MOMStatus setKeyMode(MOMControllerRef controller,
                            MOMPeerContext *peerContext,
                            MOMEvent event,
                            CFMutableArrayRef params) {
    int32_t keyNumber;
...
typedef MOMStatus (*_MOMMessageHandler)(MOMControllerRef,
                                        MOMPeerContext *peerContext,
                                        MOMEvent,
                                        CFMutableArrayRef);
...
static struct {
    ...
    [kMOMEventGetHardwareConfig] = {kMOMEventTypeHostGetRequest,
                                    getHardwareConfig},
    [kMOMEventGetSoftwareVersion] = {kMOMEventTypeHostGetRequest,
                                     getSoftwareVersion},
...
static inline bool isEventValidOnNonMaster(MOMEvent eventWithType) {
    MOMEvent event = MOMEventGetEvent(eventWithType);

    return MOMEventIsHostRequest(eventWithType) || event < kMOMEventGetKeyMode;
}

static MOMStatus sendReply(_Nonnull MOMControllerRef controller,
    ...
    if (messageBuf) {
        if (MOMEventGetEvent(eventWithType) != kMOMEventAliveRequest) {
            _MOMDebugLog(CFSTR("queued message %.*s"),
                         (int)CFDataGetLength(messageBuf) - 1,
                         (char *)CFDataGetBytePtr(messageBuf));
        }
...
MOMStatus _MOMProcessEvent(MOMControllerRef controller,
                           MOMPeerContext *peerContext,
                           MOMEvent eventWithType,
    ...
    if (!_MOMPeerIsMaster(controller, peerContext) &&
        ...
        if (handler) {
            ...
            if (status != kMOMStatusContinue &&
                MOMEventIsHostRequest(eventWithType)) {
                _MOMPeerContextRetain(peerContext);
                status = sendReply(controller, peerContext, eventWithType,
                                   status, replyParams);
        ...
        if (status == kMOMStatusContinue) {
            if (MOMEventIsHostRequest(eventWithType)) {
                _MOMPeerContextRetain(peerContext);
                status =
                    controller->handler(controller, peerContext, eventWithType,
                                        replyParams, sendReply);
                /* handler must call sendReply(), which will release peerContext
                 */
            } else {
                status = controller->handler(controller, peerContext,
                                             eventWithType, replyParams, NULL);
            }
        }
        if (status == kMOMStatusContinue)
