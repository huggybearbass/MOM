...
static inline void insertNumber(CFMutableArrayRef array, int32_t number) {
...
static inline void insertString(CFMutableArrayRef array, CFStringRef string) {
    insertStringAt(array, 0, string);
}

static inline void insertOptionAt(CFMutableArrayRef array,
                                  CFIndex index,
...
static inline void insertOption(CFMutableArrayRef array,
                                MOMControllerRef controller,
                                CFStringRef key) {
...
static inline CFStringRef getStringAt(CFArrayRef array, CFIndex index) {
    CFIndex last = CFArrayGetCount(array);
    CFTypeRef value;
...
static inline bool addressIsInList(CFDataRef address, CFArrayRef addressList) {
    CFIndex i;
    const struct sockaddr_in *sin1 =
        (const struct sockaddr_in *)CFDataGetBytePtr(address);
    bool bMatched = false;

    for (i = 0; i < CFArrayGetCount(addressList); i++) {
        const struct sockaddr_in *sin2 =
            (const struct sockaddr_in *)CFDataGetBytePtr(
                CFArrayGetValueAtIndex(addressList, i));

        if (sin2->sin_family == AF_INET &&
            memcmp(&sin1->sin_addr, &sin2->sin_addr, sizeof(struct in_addr)) ==
                0) {
            bMatched = true;
            break;
        }
