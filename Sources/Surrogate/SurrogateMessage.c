...
static MOMEvent messageTagToEventType(UniChar messageTag) {
    switch (messageTag) {
...
static UniChar eventToMessageTag(MOMEvent event) {
...
static CFStringRef _MOMMessages[kMOMEventMax + 1] = {
...
static inline void cfStringAppendCharacter(CFMutableStringRef string,
                                           UniChar c) {
    CFStringAppendCharacters(string, &c, 1);
}

CFDataRef _MOMCreateMessageFromEvent(MOMEvent event, CFArrayRef eventParams) {
    CFMutableStringRef messageBuf;

    assert(MOMEventGetEvent(event) > kMOMEventNone &&
           MOMEventGetEvent(event) <= kMOMEventMax);
    ...
    if (eventParams) {
        ...
        CFNumberFormatterRef numberFormatter =
            CFNumberFormatterCreate(kCFAllocatorDefault, CFLocaleGetSystem(),
    ...
    CFDataRef message = CFStringCreateExternalRepresentation(
        kCFAllocatorDefault, messageBuf, kCFStringEncodingUTF8, 0);
    CFRelease(messageBuf);

    return message;
}

MOMStatus _MOMParseMessageData(CFDataRef messageBuffer,
                               MOMEvent *pEvent,
    ...
    *pErrorReply = NULL;
    ...
    string = CFStringCreateWithBytes(
        kCFAllocatorDefault, CFDataGetBytePtr(messageBuffer),
        CFDataGetLength(messageBuffer), kCFStringEncodingUTF8, true);
    if (string == NULL)
...
static MOMEvent parseEventName(CFStringRef messageBuf) {
    CFIndex i;
    ...
    for (i = 0; i <= kMOMEventMax; i++) {
        if (_MOMMessages[i] && CFStringCompare(messageBuf, _MOMMessages[i],
                                               0) == kCFCompareEqualTo) {
            return i;
        }
    }
...
static UniChar getPrefixCharacter(CFStringRef string) {
    return CFStringGetLength(string) ? CFStringGetCharacterAtIndex(string, 0)
                                     : 0;
}

static void makeErrorReply(MOMEvent eventType,
    ...
    errorMessage = CFStringCreateWithFormat(
        kCFAllocatorDefault, NULL, CFSTR("%c%@,%d\r"),
        eventToMessageTag(eventType), eventName,
        (eventType == kMOMEventTypeHostGetRequest) ? 0 : 1);

    *pErrorReply = CFStringCreateExternalRepresentation(
        kCFAllocatorDefault, errorMessage, kCFStringEncodingUTF8, 0);

    CFRelease(errorMessage);
}

MOMStatus _MOMParseMessageString(CFStringRef string,
                                 MOMEvent *pEvent,
                                 CFMutableArrayRef *pEventParams,
                                 CFDataRef *pErrorReply) {
    bool ret = false;
    ...
    *pErrorReply = NULL;
    ...
    tokens = CFStringCreateArrayBySeparatingStrings(kCFAllocatorDefault, string,
                                                    CFSTR(","));
    if (tokens == NULL)
        goto out;
    ...
    eventName = CFStringCreateWithSubstring(
        kCFAllocatorDefault, token,
    ...
    numberFormatter = CFNumberFormatterCreate(
    ...
    for (i = 1; i < CFArrayGetCount(tokens); i++) {
        ...
        if (getPrefixCharacter(element) == '\'' &&
            CFStringGetCharacterAtIndex(element, CFStringGetLength(element) -
                                                     1) == '\'') {
            CFStringRef stringValue = CFStringCreateWithSubstring(
                kCFAllocatorDefault, element,
            ...
            number = CFNumberFormatterCreateNumberFromString(
                kCFAllocatorDefault, numberFormatter, element, NULL,
...
CFDataRef _MOMCreateDeviceReplyMessage(MOMEvent event, CFArrayRef eventParams) {
    ...
    return _MOMCreateMessageFromEvent(requestEvent | replyEventType,
                                      eventParams);
}
