...
#ifdef __APPLE__
static void __CFHostPerformBlock(CFHostRef host,
                                 CFHostInfoType typeInfo,
                                 const CFStreamError *error,
                                 void *info) {
    void (^block)(CFHostRef host, CFHostInfoType typeInfo,
                  const CFStreamError *error) = info;
    block(host, typeInfo, error);
}

static void CFHostSetClientBlock(CFHostRef host,
                                 void (^block)(CFHostRef host,
                                               CFHostInfoType typeInfo,
                                               const CFStreamError *error)) {
...
void _MOMResolveHostRestrictionAndPerform(
    ...
    void (^withResolvedHost)(MOMControllerRef controller,
                             CFArrayRef restrictAddressList)) {
    CFStringRef hostRestriction =
        CFDictionaryGetValue(controller->options, kMOMRestrictToSpecifiedHost);

    if (hostRestriction) {
        const char *ipAddr =
            CFStringGetCStringPtr(hostRestriction, kCFStringEncodingUTF8);
        ...
        if (ipAddr && inet_pton(AF_INET, ipAddr, &sin.sin_addr) != 0) {
            ...
            CFDataRef address =
                CFDataCreate(kCFAllocatorDefault, (uint8_t *)&sin, sizeof(sin));
            CFArrayRef addressList =
                CFArrayCreate(kCFAllocatorDefault, (const void **)&address, 1,
            ...
            CFHostRef host =
                CFHostCreateWithName(kCFAllocatorDefault, hostRestriction);
            CFStreamError streamError;

            MOMControllerRetain(controller);
            CFHostSetClientBlock(host, ^(CFHostRef host,
                                         CFHostInfoType typeInfo,
                                         const CFStreamError *error) {
              ...
              CFHostUnscheduleFromRunLoop(host, controller->runLoop,
                                          kCFRunLoopCommonModes);
              MOMControllerRelease(controller);
            });
            CFHostScheduleWithRunLoop(host, controller->runLoop,
                                      kCFRunLoopCommonModes);
